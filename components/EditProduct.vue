...
import { productsService, type ProductsDto } from '~/api/products.service'
...
const props = defineProps<{ item: ProductsDto }>()
...
const currentImage = ref<string | null>(state.imageUrl)
...
const selectItems = Object.entries(ProductCategory).map(([_, value]) => ({
	label: value,
	value: value,
}))
</script>
		...
		<template #content>
			...
			>
				...
				<div class="flex gap-2">
					<UButton type="submit" class="w-full">Сохранить изменения</UButton>
				</div>
			</UForm>
		</template>
	</UDrawer>
